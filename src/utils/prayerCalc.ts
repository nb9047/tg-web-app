...
// Tabular Islamic Calendar (Kuwaiti algorithm) for Hijri Date
export const getHijriDate = (date: Date): { day: number; monthNameUz: string; monthNameRu: string; year: number; monthIndex: number } => {
  const jd = getJulianDate(date);
  ...
  return {
    day: d,
    monthNameUz: monthsUz[m - 1] || 'Ramazon',
    monthNameRu: monthsRu[m - 1] || 'Рамадан',
    year: y,
    monthIndex: m - 1 // 8 for Ramazon
  };
};
